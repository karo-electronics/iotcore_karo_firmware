...
#if !defined(CONFIG_TARGET_KARO_TX8MM_EVK) && !defined(CONFIG_TARGET_TX8M) 
#define NXP_FUDGE 6
#else
#define NXP_FUDGE 0
#endif
...
static void sec_mipi_dsim_set_main_mode(struct sec_mipi_dsim *dsim)
{
	...
	if (dsim->dsi_panel_drv) {
		...
		hfp_wc = vmode->right_margin * (bpp >> 3) / dsim->lanes - NXP_FUDGE;
		hbp_wc = vmode->left_margin * (bpp >> 3) / dsim->lanes - NXP_FUDGE;
	}
	...
	if (dsim->dsi_panel_drv) {
		...
		hsa_wc = vmode->hsync_len * (bpp >> 3) / dsim->lanes - NXP_FUDGE;
	}
